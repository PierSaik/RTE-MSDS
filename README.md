...
3. After it is done, .off files for 3D vizualisation is created as well as the "ampl_generators_domain.txt" file describing the domain with linear constraints.
...
1. Open a console and move to the Example1 directory : "cd ~\Example1"
2. Execute the script with the desired parameters, for example : ">python ..\MachineStableDomainSearch.py -p DiagUQ-Example1.csv -t "type" -d 0 -s 1 -c 1 -a 0 -r 0"
...
2. Input the desired files (first .dta file, second generators.txt file, and third the dict.csv file)
